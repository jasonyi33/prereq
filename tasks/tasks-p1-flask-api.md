...
- `api/main.py` - Flask app entry point; registers all blueprints, runs SQLAlchemy `create_all()` on startup, configures CORS
- `api/models.py` - SQLAlchemy models for all Flask-owned tables (courses, concept_nodes, concept_edges, students, student_mastery)
- `api/db.py` - SQLAlchemy engine + session setup from `DATABASE_URL`
...
- Flask auto-creates tables on startup via `Base.metadata.create_all()`. No migrations needed.
- The mastery PUT endpoint must handle 3 body formats: `{ confidence }`, `{ eval_result }`, `{ delta }`. See CLAUDE.md "Mastery" section.
- When creating a student, eagerly bulk-insert `student_mastery` rows for every concept in the course (confidence 0.0). When uploading a PDF (creating concepts), bulk-insert mastery rows for all existing students.
...
- [ ] 1.0 Set up Flask project structure with SQLAlchemy and database schema
  - [ ] 1.1 Create `api/` directory with `main.py`, `db.py`, `models.py`, `requirements.txt`
  - [ ] 1.2 In `requirements.txt`, add: `flask`, `flask-cors`, `sqlalchemy`, `psycopg2-binary`, `anthropic`, `pdfplumber`, `python-dotenv`, `pytest`
  - [ ] 1.3 In `db.py`, set up SQLAlchemy engine and scoped session using `DATABASE_URL` from env (loaded via `python-dotenv` from `../.env`)
  - [ ] 1.4 In `models.py`, define SQLAlchemy models for all 5 Flask-owned tables: `courses`, `concept_nodes`, `concept_edges`, `students`, `student_mastery`. Match the schema exactly from CLAUDE.md (UUIDs via `gen_random_uuid()`, correct column types, foreign keys, unique constraints)
  - [ ] 1.5 In `main.py`, create Flask app, enable CORS (`flask-cors`), call `Base.metadata.create_all(engine)` on startup to auto-create tables, and set up a basic health check route `GET /api/health` → `{ status: "ok" }`
  ...
  - [ ] 1.7 Create `.env.example` at project root with all env vars (DATABASE_URL, ANTHROPIC_API_KEY, PERPLEXITY_API_KEY, FLASK_API_URL, DEMO_MODE)
  - [ ] 1.8 Verify: run `python main.py`, confirm tables are created in Postgres, health check returns 200
...
- [ ] 3.0 Implement PDF upload and concept extraction endpoint
  ...
  - [ ] 3.2 Use `pdfplumber` to extract raw text from the uploaded PDF
  - [ ] 3.3 Create `api/services/concept_extraction.py`. This calls Claude Sonnet (`claude-sonnet-4-5-20250929`) with the PDF text and the concept extraction prompt. **DEPENDENCY: Person 3 must provide the prompt text** (see task P3 1.0). Until then, use a placeholder prompt that asks Claude to extract concepts and return JSON.
  ...
  - [ ] 3.5 Bulk-insert `concept_nodes` rows from the concepts array. Build a `label → UUID` map from the inserted rows.
...
- [ ] 4.0 Implement knowledge graph query endpoint
  ...
  - [ ] 4.2 Query all `concept_nodes` for the course. If `student_id` is provided, LEFT JOIN with `student_mastery` to get confidence for each node. Derive `color` from confidence using `confidence_to_color()`.
...
- [ ] 5.0 Implement mastery endpoints (GET, PUT with 3 modes, attendance-boost)
  ...
  - [ ] 5.3 Create `api/tests/conftest.py` with pytest fixtures: test database (use a separate test DB or in-memory SQLite), test Flask client, seed helper functions
...
- [ ] 7.0 Create seed demo script
  - [ ] 7.1 Create `scripts/seed_demo.py` that uses Flask API endpoints (via HTTP requests to localhost:5000) or direct DB access to create all demo data
