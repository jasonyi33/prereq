  ...
  return (
    <div className="flex h-full flex-col rounded-2xl bg-white border border-gray-200/80 overflow-hidden">
      <div className="px-5 pt-5 pb-3">
        ...
        <p className="text-xs text-gray-400 mt-0.5 ml-4">Real-time lecture feed</p>
      </div>
      <div className="flex-1 overflow-y-auto px-5 pb-4 space-y-3">
        {chunks.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center py-12">
            ...
            <p className="text-xs text-gray-300 mt-1">Audio will appear here once the lecture starts</p>
          </div>
        )}
        {chunks.map((chunk) => (
          <div key={chunk.id} className="text-sm leading-relaxed">
            ...
            <span className="text-gray-600">{chunk.text}</span>
