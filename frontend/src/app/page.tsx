...
// --- Legacy landing page (no Supabase auth configured) ---
...
function LegacyLanding() {
  ...
  useEffect(() => {
    flaskApi
      .get("/api/courses")
      .then((courses: { id: string; name: string }[]) => {
      ...
      .then((data: { id: string; name: string }[] | undefined) => {
      ...
      <div className="relative z-10 w-full max-w-md px-6">
        ...
        <div className="rounded-2xl bg-white/70 border border-slate-200/80 backdrop-blur-xl p-8 shadow-xl shadow-slate-200/50">
          <div className="flex gap-1 mb-8 bg-slate-100 p-1 rounded-full">
            <TabButton active={mode === "student"} onClick={() => setMode("student")}>Student</TabButton>
            <TabButton active={mode === "professor"} onClick={() => setMode("professor")}>Professor</TabButton>
          </div>

          {mode === "student" ? (
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-medium text-slate-600 mb-2">Select your name</label>
                {loading ? (
                  <div className="h-10 rounded-lg bg-slate-100 animate-pulse" />
                ) : students.length === 0 ? (
                  <p className="text-sm text-slate-400 py-3">No students found. Run the seed script first.</p>
                ) : (
                  <select
              ...
              >
                {entering ? "Joining..." : "Join Lecture"}
              </GradientButton>
            </div>
          ) : (
            <div className="space-y-6">
              <div className="text-center py-4">
                <p className="text-sm text-slate-500">Access the live dashboard with heatmap, poll controls, and real-time transcript.</p>
              </div>
              <GradientButton
                onClick={() => {
                  setEntering(true);
        ...
        <p className="text-center text-xs text-slate-400 mt-8">Personalized knowledge graphs for every student</p>
      </div>
    </div>
  );
}
